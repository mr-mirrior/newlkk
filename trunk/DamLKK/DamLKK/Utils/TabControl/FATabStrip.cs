...
//                 currentItem.Size = new Size(currentItem.Size.Width, currentItem.Size.Height-17+(int)buttonRect.Height);
        ...
        public bool IsCurrent(string s)
        {
            ...
            if (!(SelectedItem.Tag is string))
                return false;
            return false;
            //string fullpath = ((DamLKK.Views.LayerView)SelectedItem.Tag).FullPath;
            //return fullpath.Equals(s, StringComparison.OrdinalIgnoreCase);
        }
        public FATabStripItem Exist(DamLKK.Views.LayerView c)
        {
        ...
        public FATabStripItem Exist(DamLKK._Model.Unit unit,DamLKK._Model.Elevation e)
        {
            foreach (FATabStripItem item in items)
            {
                if (item.Tag == null)
                    continue;
                if (((DamLKK.Views.LayerView)item.Tag).IsEqual(unit,e))
                    return item;
            }
            return null;
        }
